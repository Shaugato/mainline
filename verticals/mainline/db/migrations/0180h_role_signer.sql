...
--
-- MAINLINE · 0180h_role_signer.sql
-- CREATE ROLE IF NOT EXISTS signer — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO signer` is refused at DDL time with
--            `role/user "signer" does not exist` — measured against CockroachDB CCL
...
-- ────────────────────────────────────────────────────────────────────────────
-- A human who signs dispositions, and the role whose RESTRICTIVE policies are
-- the interesting ones: `peer_blind` on `mainline.disposition` (0185c) and
-- `standing_blind` on `mainline_meas.standing` (0187b). Access control doing
-- epistemics — it stops 'I'll sign what Dave signed', and it is what M10's
-- peer-prediction channel structurally requires. It is also why NOLOGIN
-- matters most here: a signer reaches this role through a personal role named
-- with their own `signer_sub`, and `peer_blind` compares that token to
-- CURRENT_USER.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `signer` leaves
...
CREATE ROLE IF NOT EXISTS signer WITH NOLOGIN;
