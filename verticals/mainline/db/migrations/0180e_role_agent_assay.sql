...
--
-- MAINLINE · 0180e_role_agent_assay.sql
-- CREATE ROLE IF NOT EXISTS agent_assay — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO agent_assay` is refused at DDL time with
--            `role/user "agent_assay" does not exist` — measured against CockroachDB CCL
...
--            is applied.
--
-- migration:  0180e_role_agent_assay
...
-- ────────────────────────────────────────────────────────────────────────────
-- Measurement only, confined to `mainline_meas`. It is the one role that
-- writes `mainline_meas.standing`, so it is named by both write policies on
-- that table (0187c, 0187d). Under FORCE ROW LEVEL SECURITY with no INSERT
-- policy it would fail silently and the measure would simply stop being
-- computed — which, for a mechanism that ships inert, is indistinguishable
-- from working as designed. That is the S22 trap in its quietest form.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `agent_assay` leaves
...
CREATE ROLE IF NOT EXISTS agent_assay WITH NOLOGIN;
