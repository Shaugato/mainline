...
--
-- MAINLINE · 0131_trg_cr_merge_gate.sql
-- CREATE TRIGGER cr_merge_gate ON mainline.change_request — the weld
--
-- MI: MI30
-- I: I02
-- COUNSEL-GATED: no
...
CREATE TRIGGER cr_merge_gate BEFORE UPDATE ON mainline.change_request
  FOR EACH ROW WHEN ((NEW).state = 'merged'
                     AND (OLD).state <> 'merged')
  EXECUTE FUNCTION mainline.fn_cr_merge_gate();
