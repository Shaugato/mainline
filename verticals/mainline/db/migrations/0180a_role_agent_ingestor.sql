...
--
-- MAINLINE · 0180a_role_agent_ingestor.sql
-- CREATE ROLE IF NOT EXISTS agent_ingestor — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO agent_ingestor` is refused at DDL time with
--            `role/user "agent_ingestor" does not exist` — measured against CockroachDB CCL
...
--            is applied.
--
-- migration:  0180a_role_agent_ingestor
...
-- ────────────────────────────────────────────────────────────────────────────
-- The untrusted-document plane: it turns incident PDFs into events, control
-- failures and cues. It is named by `service_read` on both gated subjects
-- (0181d, 0183d) because it holds schema-wide SELECT on `mainline` in
-- GRANTS.yaml and because the outbox trigger on the tables it writes fires as
-- the invoking role. It is also the role adversary I — the injector — reaches
-- first, which is why it holds INSERT on six tables and UPDATE on none.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `agent_ingestor` leaves
...
CREATE ROLE IF NOT EXISTS agent_ingestor WITH NOLOGIN;
