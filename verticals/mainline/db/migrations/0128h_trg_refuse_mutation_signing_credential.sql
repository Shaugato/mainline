...
--
-- MAINLINE · 0128h_trg_refuse_mutation_signing_credential.sql
-- CREATE TRIGGER append_only ON mainline.signing_credential
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
...
-- requires: 0107 mainline.fn_refuse_mutation · mainline.signing_credential
-- sqlstate: P0001, on every UPDATE and every DELETE, for every role, forever.
-- forward-only; no .down.sql and no .up.sql (MR-5).

CREATE TRIGGER append_only BEFORE UPDATE OR DELETE ON mainline.signing_credential
