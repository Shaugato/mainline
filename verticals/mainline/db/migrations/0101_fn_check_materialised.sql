...
--
-- MAINLINE · 0101_fn_check_materialised.sql
-- CREATE FUNCTION mainline.fn_check_materialised — a new obligation closes the gate and bumps the epoch
--
-- MI: MI02, MI07, MI30
-- I: I02, I03
-- COUNSEL-GATED: no
...
-- requires:   mainline.blocking_check (0058) ·
--             mainline.permit with gate_epoch and open_blocking
--             mainline.change_request with gate_epoch and open_blocking
--             mainline_ops.outbox (0099) — required because this binding declares emit_outbox
-- provides:   mainline.fn_check_materialised() — welded to blocking_check by 0121
...
--
-- ACYCLICITY. AFTER INSERT on blocking_check. It writes the gated subject tables and
-- mainline_ops.outbox, and the only trigger on a gated subject table is the merge gate,
-- whose WHEN clause restricts it to the merge transition — so this UPDATE cannot re-enter.
-- Trigger depth contributed: 1.

CREATE FUNCTION mainline.fn_check_materialised() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
...
BEGIN
  IF (NEW).permit_id IS NOT NULL THEN
    ...
    IF v_state IS NULL THEN
      RAISE EXCEPTION USING ERRCODE='P0001',
        MESSAGE='MAINLINE: no permit row for this blocking check — the gate has no subject to close';
    END IF;
  ...
  ELSIF (NEW).cr_id IS NOT NULL THEN
    SELECT t.state INTO v_state
      FROM mainline.change_request t
     WHERE t.cr_id = (NEW).cr_id
       FOR UPDATE;
    IF v_state IS NULL THEN
      RAISE EXCEPTION USING ERRCODE='P0001',
        MESSAGE='MAINLINE: no change_request row for this blocking check — the gate has no subject to close';
    END IF;
    IF v_state = 'merged' THEN
      RAISE EXCEPTION USING ERRCODE='P0001',
        MESSAGE='MAINLINE: precursor arrived after issue — use the post-issue recall path';
  ...
  ELSE
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: a blocking check names no gated subject — an obligation that blocks nothing is not an obligation';
  END IF;
