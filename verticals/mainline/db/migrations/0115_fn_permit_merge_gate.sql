...
--
-- MAINLINE · 0115_fn_permit_merge_gate.sql
-- CREATE FUNCTION mainline.fn_permit_merge_gate() — the re-derivation, the time condition, and the certified null
--
-- MI: MI02, MI22, MI06
-- I: I02, I08
-- COUNSEL-GATED: no
...
--
-- SUBJECT: permit (mainline.permit), completing state
-- 'merged'. Welded to the table by migration
-- 0130_trg_permit_merge_gate.sql, which carries the WHEN clause that
...
-- CHECK (gate_closed_when_issued) surviving when this trigger is disabled, and by
...
-- (mainline.fn_permit_merge_gate), because `diag.constraint_name` is empty for
...
CREATE FUNCTION mainline.fn_permit_merge_gate() RETURNS TRIGGER
...
BEGIN
  ...
  -- gate_closed_when_issued refuses this write with its own name attached, and
  ...
  SELECT count(*) INTO v_unbacked
    FROM mainline.permit_clause cited
   WHERE cited.permit_id = v_subject
     AND NOT EXISTS (
  ...
  -- ── 4 · THE CERTIFIED NULL (CF-53, invariant I08) ────────────────────────────
  -- unmodelled_asset_count = 0 means one of two things and they are not the same
  -- fact: CERTIFIED ZERO, or NEVER COMPUTED. No CHECK over that column can tell them
  -- apart, because both are the integer 0. So the gate demands the certificate row
  -- itself, and a permit whose boundary was never certified is refused rather than
  -- admitted. CF-52 — a certificate that exists and reports unmodelled assets — is a
  -- different refusal and belongs to boundary_certified_when_issued, which this
  -- function does not pre-empt.
  SELECT count(*) INTO v_certified
    FROM mainline.boundary_certificate cert
   WHERE cert.permit_id = v_subject;
  IF v_certified = 0 THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'MAINLINE: merge refused by mainline.fn_permit_merge_gate'
                || ' — no boundary certificate for this permit';
  END IF;
