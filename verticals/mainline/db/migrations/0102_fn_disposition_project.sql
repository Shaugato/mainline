...
--
-- MAINLINE · 0102_fn_disposition_project.sql
-- CREATE FUNCTION mainline.fn_disposition_project — identity, virulence and deliberation are projections
--
...
-- requires:   mainline.blocking_check (0058) · mainline.exposure_receipt (0061) ·
--             mainline.disposition_citation (0067) · mainline.override_ledger (0068) ·
--             mainline.clause_blame_current · mainline.clearance_legal · mainline.person ·
--             mainline_meas.recall_policy (the reading-rate policy; absent values fall back to
...
-- provides:   mainline.fn_disposition_project() — welded to disposition by 0122
...
CREATE FUNCTION mainline.fn_disposition_project() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
  ...
  v_severity INT2;
  v_virulence mainline.virulence_class;
  v_closure_gen INT8;
...
BEGIN
  ...
  IF v_subject_kind IS NULL THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: no such blocking check — a disposition cannot be filed against an obligation that does not exist';
  END IF;
  ...
  IF v_severity IS NULL THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: blame closure absent — a disposition cannot be typed';
  END IF;
  ...
  SELECT er.issued_at, er.expires_at, er.total_tokens
    ...
    FROM mainline.exposure_receipt er
   WHERE er.receipt_id = (NEW).receipt_id;
  IF v_expires_at IS NULL OR v_expires_at <= now() THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: exposure receipt absent or expired — re-materialise before signing';
  END IF;
  ...
  SELECT cl.req_compensating, cl.req_second_signer, cl.req_foreign_org, cl.req_predicate, cl.req_reassert, cl.min_signer_rank, cl.max_ttl_hours
    ...
    FROM mainline.clearance_legal cl
  ...
  IF v_signer_rank IS NULL THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: no competency record for this signer — fail closed';
  END IF;
  ...
  ELSE
    ...
    IF v_co_rank IS NULL THEN
      RAISE EXCEPTION USING ERRCODE='P0001',
        MESSAGE='MAINLINE: no competency record for this countersigner — fail closed';
    END IF;
  END IF;
  ...
  SELECT (SELECT count(*)
            FROM mainline.disposition_citation dc
           WHERE dc.disposition_id = (NEW).disposition_id
             AND dc.kind = 'verbatim')::INT4,
         (SELECT count(*)
            FROM mainline.override_ledger ol
  ...
  SELECT (rp.tau->>'tau0')::FLOAT8, (rp.tau->>'rho')::FLOAT8
    ...
    JOIN mainline.exposure_receipt er2 ON er2.policy_version = rp.policy_version
