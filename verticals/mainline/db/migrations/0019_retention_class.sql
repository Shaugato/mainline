...
--             rendering. `retention_class` is VERTICAL, not substrate (MR-2): a second
--             TRAPPOINT vertical does not need it to pass trappoint-conform, so it is authored
--             here rather than emitted from packages/trappoint-sql/templates/.
-- statements: 1
...
-- sqlstate:   23503 on destruction_record.class_id (owner: datamodel `dm-periphery`;
--             see migrations.allocation.toml for its band)
