...
--
-- MAINLINE · 0180f_role_site_reader.sql
-- CREATE ROLE IF NOT EXISTS site_reader — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO site_reader` is refused at DDL time with
--            `role/user "site_reader" does not exist` — measured against CockroachDB CCL
...
--            is applied.
--
-- migration:  0180f_role_site_reader
...
-- ────────────────────────────────────────────────────────────────────────────
-- THE RLS PRIVILEGE CARRIER, and it is the subtlest role in the matrix.
-- Policies read `USING (site_role = CURRENT_USER)`, so the actual site-scoped
-- principals are the per-site roles named by `mainline.site.site_role`, each
-- granted `site_reader` for its table privileges. `site_reader` carries the
-- PRIVILEGE; the per-site role carries the SCOPE TOKEN. A person queries AS
-- their site role and INHERITS the privilege; they never SET ROLE site_reader,
-- because that would make CURRENT_USER equal to `site_reader` and `site_scope`
-- would correctly match nothing.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `site_reader` leaves
...
CREATE ROLE IF NOT EXISTS site_reader WITH NOLOGIN;
