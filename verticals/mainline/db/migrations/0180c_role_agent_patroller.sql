...
--
-- MAINLINE · 0180c_role_agent_patroller.sql
-- CREATE ROLE IF NOT EXISTS agent_patroller — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO agent_patroller` is refused at DDL time with
--            `role/user "agent_patroller" does not exist` — measured against CockroachDB CCL
...
--            is applied.
--
-- migration:  0180c_role_agent_patroller
...
-- ────────────────────────────────────────────────────────────────────────────
-- As-documented versus as-operated fixity: patrol runs, drift findings,
-- discordance warrants. Named by `service_read` because a drift finding
-- resolves through a clause to the permits citing it. Its scans read at
-- `follower_read_timestamp()` so that patrol never contends with a merge,
-- which is also why its reads must not be filtered by a policy it does not
-- appear in.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `agent_patroller` leaves
...
CREATE ROLE IF NOT EXISTS agent_patroller WITH NOLOGIN;
