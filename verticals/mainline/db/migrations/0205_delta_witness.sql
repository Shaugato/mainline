...
--             docs/leads/algorithms.md D8, §9 (0205 reserved to W4)
--             research/05-architecture/clause-identity.md §6.2 (the nine rules)
-- requires:   0002 schema mainline · 0024 mainline.commit_obj · 0028 mainline.clause
...
--             The P0001 that makes this table load-bearing lives in 0211.
...
-- `mainline.fn_delta_witness_guard` (migration 0211, P0001). An unexplainable weakening verdict
-- does not get to exist in this database — not "is flagged", not "is logged": cannot be stored.
--
...
--   COMMIT;
--
-- The guard in 0211 is a BEFORE INSERT trigger on `mainline.clause_version`, so it can only see
-- rows that are already in the transaction when the version row arrives. Witnesses written
-- afterwards are witnesses the guard never saw, and a version row that got past the guard
-- because its explanation had not been written yet is exactly the row D8 exists to prevent.
--
...
-- names a version row that exists. The guard in 0211 enforces the direction that matters (a
...
-- (workers W8/W9, migrations 0200-0204). D8 closes one hole, names it, and does not claim the
-- others.
--
...
-- follow-on migration in the free part of this band (0213), owned by whoever needs the policy,
-- so that the projection and the policy land together rather than a column arriving with no
-- trigger behind it. Until then, access to this table is by GRANT, and that is stated rather
-- than implied.
--
-- ── `rule_id` IS A CHECK AND NOT AN ENUM ─────────────────────────────────────────────────────
-- The nine rule ids mirror `mainline_domain.contracts.RULE_IDS`, and
-- `tests/integration/algorithms/lattice/test_0205_shape.py` holds the two equal by parsing this
