...
--
-- MAINLINE · 0066_disposition.sql
-- CREATE TABLE mainline.disposition — the product, and the exhibit
--
...
CREATE TABLE mainline.disposition (
  ...
  -- ▼ PROJECTED from mainline.person (MI27). See the @authority banner above.
  ...
  -- ▼ PROJECTED from mainline.clearance_legal (MI11, ruling D3: a missing row
  ...
  -- ▲
  compensating_clause_uuid UUID NULL REFERENCES mainline.clause (clause_uuid),
  ...
  retracted_by         UUID NULL REFERENCES mainline.disposition (disposition_id),
  ...
  CONSTRAINT fk_exposure FOREIGN KEY (receipt_id, check_id)
    REFERENCES mainline.exposure_line (receipt_id, check_id),
