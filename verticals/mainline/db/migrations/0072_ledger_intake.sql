...
--             is the authority for that grant; see the OWNERSHIP note below
-- statements: 1
...
-- requires:   0001a CREATE SCHEMA mainline · 0020a mainline.site
...
--
-- ═════════════════════════════════════════════════════════════════════════════════════════════
-- OWNERSHIP. `verticals/mainline/db/migrations.allocation.toml` grants 0072-0079z to CUSTODY in
-- AUTHORED mode, and that file — not a prose band in any lead plan — is the authority. The
-- datamodel lead's 0072-0129 band is revoked (docs/leads/migration-reconciliation.md MR-3), but
-- the division of labour it encoded survives untouched: `spec/custody/ledger-schema.md` §9 says
-- custody SPECIFIES the ledger, and this file is that specification implemented, exactly as the
-- recall domain implemented its own 0080-0088.
--
-- AUTHORED, not RENDERED, and the test is MR-2's: would a second TRAPPOINT vertical need this
-- object to pass `trappoint-conform`? It would not — the custody ledger is MAINLINE's, so it is
-- written here and is never emitted by a template in `packages/trappoint-sql/templates/`. A
-- hand-authored twin of a rendered file is the failure that produced this band's reconciliation.
--
-- Two guards make ownership mechanical rather than declared: `trappoint migrate lint` rule (b)
-- fails an authored file whose number falls in a rendered band and vice versa; and if any domain
-- lands a second file for this slot, `trappoint migrate` refuses the WHOLE tree with "two files
-- claim version '0072_ledger_intake'" — which is the correct outcome and is why the collision is
-- loud rather than silent. Undeclared band-borrowing is what made the last one quiet.
-- ═════════════════════════════════════════════════════════════════════════════════════════════
--
-- ═════════════════════════════════════════════════════════════════════════════════════════════
...
-- 0020a is normative and emphatic — `mainline.site` is THE authoritative source for every
