...
--
-- MAINLINE · 0180d_role_agent_fleet.sql
-- CREATE ROLE IF NOT EXISTS agent_fleet — a DDL-time dependency of the policy band, and nothing more
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: `CREATE POLICY … TO agent_fleet` is refused at DDL time with
--            `role/user "agent_fleet" does not exist` — measured against CockroachDB CCL
...
--            is applied.
--
-- migration:  0180d_role_agent_fleet
...
-- ────────────────────────────────────────────────────────────────────────────
-- Cross-site lesson propagation, under MI23: only tightenings travel. Named by
-- `service_read` on both gated subjects because a propagation resolves to a
-- target subject at another site — which is precisely the read that
-- `site_scope` would filter to nothing if this role were left out of the
-- policy.
--
-- ────────────────────────────────────────────────────────────────────────────
...
-- than applied once. Nothing in this file grants anything. `agent_fleet` leaves
...
CREATE ROLE IF NOT EXISTS agent_fleet WITH NOLOGIN;
