...
--
-- MAINLINE · 0103_fn_disposition_close.sql
-- CREATE FUNCTION mainline.fn_disposition_close — one signed disposition closes exactly one obligation
--
-- MI: MI02, MI29, MI30
-- I: I02, I10
-- COUNSEL-GATED: no
...
-- requires:   mainline.disposition (0066) · mainline.override_ledger (0068) ·
--             mainline.permit (open_blocking, unmet_floor_count, countersigned_count)
--             mainline.change_request (open_blocking)
-- provides:   mainline.fn_disposition_close() — welded to disposition by 0123
...
CREATE FUNCTION mainline.fn_disposition_close() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
BEGIN
  ...
  ELSE
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='MAINLINE: a disposition names no gated subject — nothing to close';
  END IF;
  ...
  IF (NEW).kind = 'emergency_override' THEN
    INSERT INTO mainline.override_ledger
