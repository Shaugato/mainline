...
--
-- MAINLINE · 0118_proc_merge_change_request.sql
-- CREATE OR REPLACE PROCEDURE mainline.merge_change_request() — the gate transaction as one server-side statement
--
-- MI: MI09, MI30
-- I: I03, I04
-- COUNSEL-GATED: no
...
--
-- SUBJECT: change_request (mainline.change_request → mainline.cr_event → mainline.merge_record).
--
...
CREATE OR REPLACE PROCEDURE mainline.merge_change_request(
  a_subject_id    UUID,
...
BEGIN
  ...
  IF v_state IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'MAINLINE: merge refused by mainline.merge_change_request'
  ...
  -- mainline.fn_cr_merge_gate when it does not — conformance cases CF-01,
  ...
  -- ── 6 · PIN THE EPOCH ────────────────────────────────────────────────────────
  -- The composite foreign key (cr_id, gate_epoch) → mainline.change_request under
  ...
  IF v_site_code IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'MAINLINE: merge refused by mainline.merge_change_request'
                || ' — no site record for this change_request';
  END IF;
  ...
  -- ── 8 · THE LAST WRITE ───────────────────────────────────────────────────────
  -- Every CHECK on mainline.change_request fires HERE, on the completing row, and
