...
--
-- TRAPPOINT_REF · 0116_fn_cr_merge_gate.sql
-- CREATE FUNCTION trappoint_ref.fn_cr_merge_gate() — the re-derivation, the time condition, and the certified null
--
-- MI: MI02, MI22
-- I: I02
-- COUNSEL-GATED: no
...
--
-- SUBJECT: change_request (trappoint_ref.change_request), completing state
...
-- (trappoint_ref.fn_cr_merge_gate), because `diag.constraint_name` is empty for
...
CREATE FUNCTION trappoint_ref.fn_cr_merge_gate() RETURNS TRIGGER
...
BEGIN
  ...
  IF v_derived <> 0 AND v_projected = 0 THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'TRAPPOINT_REF: merge refused by trappoint_ref.fn_cr_merge_gate'
  ...
  SELECT count(*) INTO v_unbacked
    FROM trappoint_ref.cr_clause cited
