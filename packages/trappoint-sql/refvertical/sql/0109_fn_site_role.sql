...
--
-- TRAPPOINT_REF · 0109_fn_site_role.sql
-- CREATE FUNCTION trappoint_ref.fn_site_role — the tenancy token is projected, never supplied
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
...
-- requires:   trappoint_ref.site (site_id, site_role)
-- provides:   trappoint_ref.fn_site_role() — DELIBERATELY UNWELDED IN THIS BAND. See the
...
CREATE FUNCTION trappoint_ref.fn_site_role() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
  v_site_role NAME;
BEGIN
  ...
  IF v_site_role IS NULL THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='TRAPPOINT_REF: no site row for this record — the tenancy scope token has no source';
  END IF;
