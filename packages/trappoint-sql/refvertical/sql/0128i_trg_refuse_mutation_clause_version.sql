...
--
-- TRAPPOINT_REF · 0128i_trg_refuse_mutation_clause_version.sql
-- CREATE TRIGGER append_only ON trappoint_ref.clause_version
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: Weld 10 of 11 in the append-only family: MI15: editing sev_max is a shorter
--            path to a laundered ancestry than authoring a child. UPDATE and DELETE are one
...
-- requires: 0107 trappoint_ref.fn_refuse_mutation · trappoint_ref.clause_version
-- sqlstate: P0001, on every UPDATE and every DELETE, for every role, forever.
-- forward-only; no .down.sql and no .up.sql (MR-5).

CREATE TRIGGER append_only BEFORE UPDATE OR DELETE ON trappoint_ref.clause_version
