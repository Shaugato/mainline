...
--
-- TRAPPOINT_REF · 0130_trg_permit_merge_gate.sql
-- CREATE TRIGGER permit_merge_gate ON trappoint_ref.permit — the weld
--
-- MI: MI02
-- I: I02
-- COUNSEL-GATED: no
...
--
-- SUBJECT: permit. Fires only on the transition INTO 'merged', and only
...
-- `gate_closed_when_issued` rather than a P0001 from the trigger. An AFTER trigger
-- would fire only once the CHECKs had already passed, which would make the
-- re-derivation unable to refuse anything at all.

CREATE TRIGGER permit_merge_gate BEFORE UPDATE ON trappoint_ref.permit
  FOR EACH ROW WHEN ((NEW).state = 'merged'
                     AND (OLD).state <> 'merged')
  EXECUTE FUNCTION trappoint_ref.fn_permit_merge_gate();
