...
--
-- TRAPPOINT_REF · 0115_fn_permit_merge_gate.sql
-- CREATE FUNCTION trappoint_ref.fn_permit_merge_gate() — the re-derivation, the time condition, and the certified null
--
-- MI: MI02, MI22
-- I: I02
-- COUNSEL-GATED: no
...
--
-- SUBJECT: permit (trappoint_ref.permit), completing state
...
-- (trappoint_ref.fn_permit_merge_gate), because `diag.constraint_name` is empty for
...
CREATE FUNCTION trappoint_ref.fn_permit_merge_gate() RETURNS TRIGGER
...
BEGIN
  ...
  IF v_derived <> 0 AND v_projected = 0 THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'TRAPPOINT_REF: merge refused by trappoint_ref.fn_permit_merge_gate'
  ...
  --   authority trappoint_ref.clause_blame_current (clause_uuid, as_of_commit)
  ...
  SELECT count(*) INTO v_unbacked
    FROM trappoint_ref.permit_clause cited
   WHERE cited.permit_id = v_subject
     AND NOT EXISTS (
           SELECT 1 FROM trappoint_ref.clause_blame_current auth
  ...
  -- This arm reads trappoint_ref.boundary_certificate, which this binding does not
  -- declare and does not own (`emit_outbox = false`, ruling D9). Stored
