...
--
-- TRAPPOINT_REF · 0117_proc_merge_permit.sql
-- CREATE OR REPLACE PROCEDURE trappoint_ref.merge_permit() — the gate transaction as one server-side statement
--
-- MI: MI09, MI24
-- I: I03, I04
-- COUNSEL-GATED: no
...
--
-- SUBJECT: permit (trappoint_ref.permit → trappoint_ref.permit_event → trappoint_ref.merge_record).
--
...
CREATE OR REPLACE PROCEDURE trappoint_ref.merge_permit(
  a_subject_id    UUID,
...
BEGIN
  ...
  IF v_state IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = 'TRAPPOINT_REF: merge refused by trappoint_ref.merge_permit'
  ...
  -- trappoint_ref.fn_permit_merge_gate when it does not — conformance cases CF-01,
  ...
  INSERT INTO trappoint_ref.permit_event
  ...
  -- ── 6 · PIN THE EPOCH ────────────────────────────────────────────────────────
  -- The composite foreign key (permit_id, gate_epoch) → trappoint_ref.permit under
  ...
  -- ── 8 · THE LAST WRITE ───────────────────────────────────────────────────────
  -- Every CHECK on trappoint_ref.permit fires HERE, on the completing row, and
