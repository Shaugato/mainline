...
--
-- TRAPPOINT_REF · 0060_cr_event.sql
-- CREATE TABLE trappoint_ref.cr_event — a chain, not a tree
--
-- MI: MI09, MI10, MI24
-- I: I04, I01
-- COUNSEL-GATED: no
...
--                stored predecessor by trappoint_ref.fn_cr_event_chain, never trusted
...
CREATE TABLE trappoint_ref.cr_event (
  cr_id         UUID   NOT NULL,
  seq           INT8   NOT NULL,
  prev_seq      INT8   NOT NULL,
  from_state    trappoint_ref.subject_state NOT NULL,
  to_state      trappoint_ref.subject_state NOT NULL,
  subject_kind  STRING NOT NULL DEFAULT 'change_request',
  ...
  CONSTRAINT cr_event_kind_pinned CHECK (subject_kind = 'change_request'),
  CONSTRAINT cr_event_seq_ordered CHECK (seq > prev_seq AND prev_seq >= 0),
  CONSTRAINT cr_event_prev_digest_sized CHECK (length(prev_digest) = 32),
  CONSTRAINT cr_event_actor_stated CHECK (actor_sub <> ''),
  CONSTRAINT fk_cr_event_subject FOREIGN KEY (cr_id)
    REFERENCES trappoint_ref.change_request (cr_id)
  ...
  CONSTRAINT cr_legal_edge FOREIGN KEY (subject_kind, from_state, to_state)
    REFERENCES trappoint_ref.subject_transition (subject_kind, from_state, to_state)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT pk_cr_event PRIMARY KEY (cr_id, seq),
  -- THE COMPARE-AND-SWAP. A chain, not a tree.
  CONSTRAINT cr_linear UNIQUE (cr_id, prev_seq)
);
