...
--
-- TRAPPOINT_REF · 0128g_trg_refuse_mutation_person.sql
-- CREATE TRIGGER append_only ON trappoint_ref.person
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
-- RATIONALE: Weld 8 of 11 in the append-only family: a promotion is a new row; editing rank
--            rewrites what the gate enforced. UPDATE and DELETE are one trigger because a
...
-- requires: 0107 trappoint_ref.fn_refuse_mutation · trappoint_ref.person
-- sqlstate: P0001, on every UPDATE and every DELETE, for every role, forever.
-- forward-only; no .down.sql and no .up.sql (MR-5).

CREATE TRIGGER append_only BEFORE UPDATE OR DELETE ON trappoint_ref.person
