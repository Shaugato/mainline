...
--
-- TRAPPOINT_REF · 0119a_fn_explain_refusal.sql
...
BEGIN
  ...
  IF p_subject_kind = 'permit' THEN
    SELECT sub.gate_epoch, sub.state::STRING
      ...
      FROM trappoint_ref.permit AS sub
     WHERE sub.permit_id = p_subject_id;
  END IF;
  IF p_subject_kind = 'change_request' THEN
    SELECT sub.gate_epoch, sub.state::STRING
      INTO v_epoch, v_state
      FROM trappoint_ref.change_request AS sub
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    WITH open_obl AS (
      SELECT bc.check_id,
             ...
             bc.severity,
             bc.virulence
        FROM trappoint_ref.blocking_check AS bc
       WHERE bc.permit_id = p_subject_id
         AND NOT EXISTS (
               SELECT 1
                 FROM trappoint_ref.disposition AS d
      ...
      SELECT cl.kind
        FROM trappoint_ref.clearance_legal AS cl
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.permit.open_residue',
               'required_value', 0,
    ...
    v_naa := jsonb_build_object(
               ...
               'required', jsonb_build_array('trappoint_ref.permit.open_residue'),
               'cardinality', v_value,
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.permit.open_conflicts',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
    ...
    v_naa := jsonb_build_object(
               'kind', 'supply_evidence',
               'required', jsonb_build_array('trappoint_ref.permit.open_conflicts'),
               'cardinality', v_value,
               'description', v_value::STRING
                              || ' obligation(s) feed this counter; resolving exactly those '
                              || 'restores admissibility');
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.permit.open_warrants',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
    ...
    v_naa := jsonb_build_object(
               'kind', 'supply_evidence',
               'required', jsonb_build_array('trappoint_ref.permit.open_warrants'),
               'cardinality', v_value,
               'description', v_value::STRING
                              || ' obligation(s) feed this counter; resolving exactly those '
                              || 'restores admissibility');
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.permit.unmodelled_asset_count',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
    ...
    v_naa := jsonb_build_object(
               'kind', 'supply_evidence',
               'required', jsonb_build_array('trappoint_ref.permit.unmodelled_asset_count'),
               'cardinality', v_value,
               'description', v_value::STRING
                              || ' obligation(s) feed this counter; resolving exactly those '
                              || 'restores admissibility');
  ...
  IF NOT v_handled AND p_subject_kind = 'permit'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.permit.unmet_floor_count',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
  ...
  IF NOT v_handled AND p_subject_kind = 'change_request'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.change_request.open_residue',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
    ...
    v_naa := jsonb_build_object(
               'kind', 'supply_evidence',
               'required', jsonb_build_array('trappoint_ref.change_request.open_residue'),
               'cardinality', v_value,
               'description', v_value::STRING
                              || ' obligation(s) feed this counter; resolving exactly those '
                              || 'restores admissibility');
  ...
  IF NOT v_handled AND p_subject_kind = 'change_request'
    ...
    v_mus := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
               'kind', 'capability_gap',
               'capability', 'trappoint_ref.change_request.open_conflicts',
               'required_value', 0,
               'observed_value', v_value,
               'detail', 'projected counter is non-zero; its source relation is declared by '
                         || 'the vertical, so the substrate names the counter rather than '
    ...
    v_naa := jsonb_build_object(
               'kind', 'supply_evidence',
               'required', jsonb_build_array('trappoint_ref.change_request.open_conflicts'),
               'cardinality', v_value,
               'description', v_value::STRING
                              || ' obligation(s) feed this counter; resolving exactly those '
                              || 'restores admissibility');
  ...
  IF NOT v_handled AND p_constraint = 'fk_clearance' THEN
    ...
    IF v_open_n = 0 THEN
      ...
      v_mus := jsonb_build_array(jsonb_build_object(
                 'kind', 'capability_gap',
                 'capability', 'trappoint_ref.clearance_legal',
