...
--
-- TRAPPOINT_REF · 0128c_trg_refuse_mutation_merge_record.sql
-- CREATE TRIGGER append_only ON trappoint_ref.merge_record
--
-- MI: MI01
-- I: I01
-- COUNSEL-GATED: no
...
-- requires: 0107 trappoint_ref.fn_refuse_mutation · trappoint_ref.merge_record
-- sqlstate: P0001, on every UPDATE and every DELETE, for every role, forever.
-- forward-only; no .down.sql and no .up.sql (MR-5).

CREATE TRIGGER append_only BEFORE UPDATE OR DELETE ON trappoint_ref.merge_record
