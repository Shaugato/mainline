...
--
-- TRAPPOINT_REF · 0071b_epoch_pin_cr.sql
-- ALTER TABLE trappoint_ref.merge_record ADD CONSTRAINT epoch_pin_cr — the pin
--
-- MI: MI07
-- I: I03
-- COUNSEL-GATED: no
...
-- `cr_id` is non-null, which is exactly the rows completing a
-- change_request. A row for the other kind leaves the column NULL and the constraint does
-- not apply to it. That is the "partial-FK pair, one per kind" of §5.5, spelled out.

ALTER TABLE trappoint_ref.merge_record
  ADD CONSTRAINT epoch_pin_cr
  FOREIGN KEY (cr_id, gate_epoch)
  REFERENCES trappoint_ref.change_request (cr_id, gate_epoch)
