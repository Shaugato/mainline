...
--
-- TRAPPOINT_REF · 0105_fn_permit_event_chain.sql
-- CREATE FUNCTION trappoint_ref.fn_permit_event_chain — the permit event chain is verified against its predecessor
--
-- MI: MI01, MI09, MI24
-- I: I04
-- COUNSEL-GATED: no
...
--
-- migration:  0105_fn_permit_event_chain
...
--             DIGEST half; `UNIQUE (permit_id, prev_seq)` guards the FORK half and this
--             function deliberately does not pre-empt it)
-- source:     ARCHITECTURE.md §5.11 item 8 · §2.2 S9 · §5.5
-- requires:   trappoint_ref.permit_event (migration 0059,
--             RENDERED, kernel `subject-and-pin`) with columns seq, prev_seq, prev_digest and
--             the STORED chain_digest
-- provides:   trappoint_ref.fn_permit_event_chain() — welded to permit_event by 0125
...
--
-- ACYCLICITY. BEFORE INSERT on permit_event, reading one earlier row of the same table by
-- primary key and writing nothing. Trigger depth contributed: 0.

CREATE FUNCTION trappoint_ref.fn_permit_event_chain() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
  v_existing INT8;
  v_expected BYTES;
BEGIN
  ...
  SELECT count(*) INTO v_existing
    FROM trappoint_ref.permit_event e0
  ...
  SELECT e.chain_digest INTO v_expected
    FROM trappoint_ref.permit_event e
