...
--
-- TRAPPOINT_REF · 0108_fn_closure_guard.sql
-- CREATE FUNCTION trappoint_ref.fn_closure_guard — generations dense, severity monotone, every write ledgered
--
-- MI: MI26, MI15, MI01
-- I: I05
-- COUNSEL-GATED: no
...
--             half is `tref_projector` holding INSERT on this table and nothing else.
-- source:     ARCHITECTURE.md §5.11 item 6 · §2.2 S2 · docs/leads/custody.md A10
-- requires:   trappoint_ref.clause_blame_closure · trappoint_ref.site (site_code) ·
--             trappoint_ref.ledger_intake · trappoint_ref.event_severity_revision
-- provides:   trappoint_ref.fn_closure_guard() — welded to clause_blame_closure by 0127
...
-- grants:     tref_projector needs INSERT on clause_blame_closure and on ledger_intake, and
...
CREATE FUNCTION trappoint_ref.fn_closure_guard() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
...
BEGIN
  ...
  SELECT c.max_severity, c.closure_gen
    ...
    FROM trappoint_ref.clause_blame_closure c
  ...
  IF v_prev_severity IS NOT NULL AND (NEW).max_severity < v_prev_severity THEN
    SELECT count(*) INTO v_revisions
      FROM trappoint_ref.event_severity_revision r
  ...
  SELECT st.site_code INTO v_site_code
    FROM trappoint_ref.site st
   WHERE st.site_id = (NEW).site_id;
  IF v_site_code IS NULL THEN
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='TRAPPOINT_REF: no site row for this closure — the ledger partition key has no source';
  END IF;
  ...
  INSERT INTO trappoint_ref.ledger_intake
