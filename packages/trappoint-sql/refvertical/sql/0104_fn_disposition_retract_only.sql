...
--
-- TRAPPOINT_REF · 0104_fn_disposition_retract_only.sql
-- CREATE FUNCTION trappoint_ref.fn_disposition_retract_only — one column, one direction, once
--
-- MI: MI01, MI07
-- I: I01, I03
-- COUNSEL-GATED: no
...
-- requires:   trappoint_ref.disposition (0066) ·
--             trappoint_ref.permit (open_blocking, gate_epoch)
--             trappoint_ref.change_request (open_blocking, gate_epoch)
-- provides:   trappoint_ref.fn_disposition_retract_only() — welded to disposition by 0124
...
CREATE FUNCTION trappoint_ref.fn_disposition_retract_only() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
BEGIN
  ...
  ELSE
    RAISE EXCEPTION USING ERRCODE='P0001',
      MESSAGE='TRAPPOINT_REF: a disposition names no gated subject — nothing to re-open';
  END IF;
  RETURN NEW;
END $$;
