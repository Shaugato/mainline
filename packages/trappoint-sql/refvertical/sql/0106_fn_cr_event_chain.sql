...
--
-- TRAPPOINT_REF · 0106_fn_cr_event_chain.sql
-- CREATE FUNCTION trappoint_ref.fn_cr_event_chain — the change_request event chain is verified against its predecessor
--
-- MI: MI01, MI09, MI24
-- I: I04
-- COUNSEL-GATED: no
...
-- requires:   trappoint_ref.cr_event (migration 0060,
--             RENDERED, kernel `subject-and-pin`) with columns seq, prev_seq, prev_digest and
--             the STORED chain_digest
-- provides:   trappoint_ref.fn_cr_event_chain() — welded to cr_event by 0126
...
CREATE FUNCTION trappoint_ref.fn_cr_event_chain() RETURNS TRIGGER LANGUAGE PLpgSQL AS $$
DECLARE
  v_existing INT8;
  v_expected BYTES;
BEGIN
  ...
  SELECT count(*) INTO v_existing
    FROM trappoint_ref.cr_event e0
