    ...
    for scan_root in _resolve_scan_roots(root, scan_roots):
        for path in iter_python_files(scan_root):
            ...
            if exempt is not None:
                report.exempt(rule="GREP-SAMPLING-PARAM", subject=where, reason=exempt)
                continue
            report.examine()
            try:
                tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
                ...
                continue
            for node in ast.walk(tree):
                for key, lineno in _sampling_sites(node):
                    report.violate(
                        rule="GREP-SAMPLING-PARAM",
                        subject=f"{where}:{lineno}",
                        detail=(
                            f"model request builder sets {key!r}. A6: sampling parameters "
                            "return 400 on this generation, and the claim was always "
                            "replayability plus arithmetic reproducibility, never "
...
def _sampling_sites(node: ast.AST) -> list[tuple[str, int]]:
    ...
    if isinstance(node, ast.Call):
        ...
        for key in node.keys:
            if isinstance(key, ast.Constant) and key.value in SAMPLING_KEYS:
                out.append((str(key.value), key.lineno))
    elif isinstance(node, ast.Subscript):
        index = node.slice
        if (
            isinstance(index, ast.Constant)
            and isinstance(index.value, str)
            and index.value in SAMPLING_KEYS
        ):
            out.append((index.value, node.lineno))
    return out
            ...
            for pattern in claim.patterns:
                for match in pattern.finditer(text):
                    ...
                    report.violate(
                        ...
                        subject=f"{where}:{lineno}",
                        detail=(
                            f"{claim.description} — matched {match.group(0).strip()!r}"
                        ),
