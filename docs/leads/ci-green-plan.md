...
| **ci → REUSE** | `REFUSED [NO-LICENCE-TEXT] FSL-1.1-ALv2.` — a **trailing full stop** is being parsed into the identifier. Source: `verticals/mainline/apps/console/capture-plan.demo.json:2`, whose `$comment` reads `… SPDX-License-Identifier: FSL-1.1-ALv2. The capture plan for …`. Also `non_spdx_spelling.FSL-1.1-ALv2 baseline=1213 measured=1254` — 41 new files use the bare spelling where the ratchet expects `LicenseRef-FSL-1.1-ALv2` | **a** | w10 |
