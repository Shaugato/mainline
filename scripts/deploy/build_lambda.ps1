...
.PARAMETER ConsoleTransport
    REQUIRED. What the console artefact in `dist/` is meant to DO ON LOAD. The packer
    refuses a dist that does something else.

        live      dist/ starts LIVE: a NON-EMPTY VITE_MAINLINE_API_BASE is compiled in. A
                  dist carrying BOTH sources satisfies this, because
                  src/app/source-select.ts starts LIVE when both are present.
    ...
    VITE_MAINLINE_API_BASE="" and VITE_MAINLINE_BUNDLE_URL="./bundle/", so every byte a
    judge saw was a recorded EvidenceBundle rather than the kernel the page was sitting on
    -- and the packaging check that should have caught it keyed on the variable NAME and
    never on its VALUE, which made it unreachable. This flag is what the fixed check
    compares against. A guard that infers intent has to be right about intent; this one
    compares two things a human wrote down. See docs/leads/console-live-plan.md ruling R4.

.EXAMPLE
    pwsh scripts/deploy/build_lambda.ps1 -ConsoleTransport replay
    pwsh scripts/deploy/build_lambda.ps1 -ConsoleTransport live
    pwsh scripts/deploy/build_lambda.ps1 -ConsoleTransport replay -Arch x86_64
    pwsh scripts/deploy/build_lambda.ps1 -ConsoleTransport replay -KeepSourceMaps -KeepStage
#>
...
param(
    ...
    # REQUIRED, and deliberately without a default. ValidateSet rejects a typo; the empty
    # default is caught below with the paragraph that explains what the declaration is for,
    # because "Cannot validate argument" would not tell an operator why this flag exists.
    [ValidateSet('', 'live', 'replay', 'both')]
    [string]$ConsoleTransport = ''
)
...
# runs before the interpreter is located, before pip, before anything is staged. The POSIX
# twin refuses in the same place with the same words.
if ([string]::IsNullOrWhiteSpace($ConsoleTransport)) {
    Fail (
        "-ConsoleTransport is REQUIRED and must be live, replay or both.`n" +
        "   It declares what the console artefact in dist/ is meant to DO ON LOAD, so this build can`n" +
        "   refuse a dist that does something else. It is not inferred: on 2026-08-14 the deployed`n" +
        "   artefact carried VITE_MAINLINE_API_BASE=`"`" and VITE_MAINLINE_BUNDLE_URL=`"./bundle/`",`n" +
        "   so every byte a judge saw was a recorded EvidenceBundle rather than the kernel the page`n" +
        "   was sitting on, and the packaging step printed a cheerful line and packaged it anyway.`n" +
        "   See Get-Help, and docs/leads/console-live-plan.md ruling R4."
    )
}
...
$CommandLine = "scripts/deploy/build_lambda.ps1 -Arch $Arch -ConsoleTransport $ConsoleTransport"
...
Step "transport $ConsoleTransport (declared; the packer refuses a dist that does otherwise)"
